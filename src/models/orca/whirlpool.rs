...
impl OrcaWhirlpoolLiquidityIncreasedEvent {
    /// Deserialize from bytes, assuming the discriminator has already been checked
...
impl OrcaWhirlpoolLiquidityDecreasedEvent {
    /// Deserialize from bytes, assuming the discriminator has already been checked
    pub fn try_from_bytes(data: &[u8]) -> Result<Self, borsh::maybestd::io::Error> {
        Self::try_from_slice(data)
    }
